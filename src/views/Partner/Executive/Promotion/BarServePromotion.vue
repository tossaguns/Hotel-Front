<template>
  <div>
    <div
      class="bg-white p-3 lg:p-4 z-40 shadow-lg flex-col md:space-y-4 space-y-2 min-h-screen overflow-y-auto custom-scrollbar">
      <h2 class="text-lg mt-6 text-center text-gray-400 font-bold">ประวัติการทำรายการ</h2>
      <div v-for="(promo, index) in promotions" :key="index" class="bg-stone-100 py-3 rounded-lg shadow space-y-2">
        ...
        <div class="text-center mb-4">
          ...
          <p class="text-xs text-gray-500">ใช้งานได้ถึง : {{ promo.end }}</p>
        </div>

        <div class="flex justify-center text-xs">
          <button class="bg-stone-500 px-2 py-1 rounded text-white" @click="toggleDetail(index)">
        ...
        <transition name="fade">
          <div v-if="openedIndex === index"
            ...
            <div class="mt-5 space-x-2 flex">
              <button
                class="bg-amber-500 w-full text-white text-xs font-bold px-2 py-1 rounded-md">ใบเสร็จกำกับภาษี</button>
              <button
                class="bg-rose-500 w-full text-white text-xs font-bold px-2 py-1 rounded-md">ใบเสร็จเเบบย่อ</button>
              <button class="bg-blue-500 w-full text-white text-xs font-bold px-2 py-1 rounded-md">สลิป</button>
            </div>
          </div>
        </transition>
      </div>
