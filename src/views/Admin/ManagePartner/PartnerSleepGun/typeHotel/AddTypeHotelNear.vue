<template>
    <!--================ TypeHotelNear ======================-->
    <div>
        <div class="mb-4 text-lg font-bold text-fuchsia-700">
            <p>6. ที่พักใกล้ที่ไหนบ้าง</p>
        </div>
        ...
        <div class="max-w-5xl mx-auto border rounded-lg shadow-md">
            <div class="px-4 py-6 ">
                <form @submit.prevent="createFacility" class="space-y-4 mb-8">
                    <div>
                        <label class="font-semibold">ที่พักใกล้กับ</label>
                        <input v-model="addForm.name" type="text"
                            class="border w-full py-2 px-2 rounded-md mt-1 border-stone-300 hover:border-fuchsia-600"
                    ...
                    <div class="w-full  flex justify-center items-center flex-col space-y-4 pt-3">
                        <div class="w-32 h-32 relative">
                            ...
                            <!-- input ที่ซ่อน -->
                            <input id="add-file-upload" type="file" accept="image/*" @change="onAddFileChange"
                                ref="addFileInput" class="hidden" />
                        ...
                        <div>
                            <button type="submit" class="bg-fuchsia-600 rounded-lg 
                        px-4 py-2 text-white hover:bg-fuchsia-700 transition">เพิ่มที่พักใกล้กับ</button>
                        </div>
                    </div>
                </form>
                ...
                <div v-if="isEditing" ref="editFormSection"
                    class="mt-8 border-t max-w-2xl mx-auto rounded-lg border px-4 py-5 mb-8 shadow-lg bg-fuchsia-50">
                    <h2 class="text-xl font-semibold mb-2 text-fuchsia-700 text-center">แก้ไขที่พักใกล้กับที่ไหนบ้าง
                    </h2>
                    <form @submit.prevent="updateFacility" class="space-y-4">
                        <div>
                            <label class="font-semibold text-sm">ที่พักใกล้กับ</label>
                        ...
                        <div>
                            ...
                            <div class="flex justify-center flex-coll bg-white w-full rounded-lg py-4">
                                <div class="relative w-40">
                                    ...
                                    <input type="file" accept="image/*" @change="onEditFileChange" ref="editFileInput"
                                        class="mt-1" />

                                    <!-- รูป Preview -->
                                    <div v-if="editPreview" class="mt-2 relative">
                                        <img :src="editPreview" alt="Icon Preview"
                        ...
                        <div class="pt-10 space-x-2 flex justify-center">
                            <button type="submit" class="bg-fuchsia-400 rounded-lg 
                        px-4 py-2  hover:bg-fuchsia-500 transition">อัปเดตข้อมูล</button>
                ...
                <div>
                    <div class="hidden md:block">
                        <table class="min-w-full shadow-md">
                            <thead>
                                <tr class="bg-fuchsia-700 text-white text-center text-sm">
                            ...
                            <tbody>
                                <tr v-for="(item, index) in paginated" :key="item._id"
                                    :class="index % 2 === 0 ? 'bg-white' : 'bg-fuchsia-50'" class="border-t">
                                    <td class="p text-center align-middle">{{ item.name }}</td>
                                    <td
                                        class="p-2 text-gray-500 text-sm text-left break-words whitespace-pre-line max-w-md">
                                        {{ item.description }}
                                    </td>
                                    <td class="p-2">
                ...
                <div class="mt-4 flex justify-center space-x-2 text-gray-400">
                    <button class="" :disabled="currentPage === 1" @click="currentPage--">
                        <p class="text-xs mr-3 hover:text-fuchsia-900 hover:text-sm transition">ก่อนหน้า</p>
                    </button>

                    <button v-for="page in totalPages" :key="page" class="px-2 rounded-full "
                        :class="page === currentPage ? 'bg-fuchsia-700 text-white hover:bg-fuchsia-600 transition' : 'bg-gray-100'"
                        @click="goToPage(page)">
                        {{ page }}
                    </button>

                    <button class="" :disabled="currentPage === totalPages" @click="currentPage++">
                        <p class="text-xs ml-3 hover:text-fuchsia-900 hover:text-sm transition">ถัดไป</p>
                    </button>
                </div>
            </div>
        </div>
...
const fetch = async () => {
    try {
        const res = await axios.get('http://localhost:9999/SleepGun/typeHotelNear/getAll')
        item.value = res.data
        currentPage.value = 1
    } catch (err) {
        console.error(err)
...
const createFacility = async () => {
    try {
        ...
        await axios.post('http://localhost:9999/SleepGun/typeHotelNear/create', fd)
        alert('เพิ่มสิ่งอำนวยความสะดวกเรียบร้อยแล้ว')
        resetForm()
        fetch()
    } catch (err) {
        console.error(err)
...
const updateFacility = async () => {
    try {
        ...
        await axios.put(`http://localhost:9999/SleepGun/typeHotelNear/update/${selectedId.value}`, fd)
        alert('อัปเดตสิ่งอำนวยความสะดวกเรียบร้อยแล้ว')
        resetForm()
        fetch()
    } catch (err) {
        console.error(err)
...
const deleteFacility = async (id) => {
    ...
    try {
        await axios.delete(`http://localhost:9999/SleepGun/typeHotelNear/delete/${id}`)
        fetch()
    } catch (err) {
        console.error(err)
    }
