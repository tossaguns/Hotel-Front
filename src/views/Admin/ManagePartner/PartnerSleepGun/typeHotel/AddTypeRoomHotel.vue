<template>
    ...
    <div>
        <div class="mb-4 text-lg font-bold text-stone-700">
            <p>3. สิ่งอำนวยความสะดวกภายในห้อง</p>
        </div>
...
const createFacility = async () => {
    try {
        ...
        await axios.post('http://localhost:9999/SleepGun/typeRoomHotel/create', fd)
...
const updateFacility = async () => {
    try {
        ...
        await axios.put(`http://localhost:9999/SleepGun/typeRoomHotel/update/${selectedId.value}`, fd)
...
const deleteFacility = async (id) => {
    ...
    try {
        await axios.delete(`http://localhost:9999/SleepGun/typeRoomHotel/delete/${id}`)
        fetch()
    } catch (err) {
        console.error(err)
    }
