<template>
    <!--================ TypeHotel ======================-->
    <div>
        <div class="mb-4 text-lg font-bold text-cyan-700">
            <p>1. ประเภทที่พัก</p>
        </div>
        ...
        <div class="max-w-5xl mx-auto border rounded-lg shadow-md">
            <div class="px-4 py-6 ">
                <form @submit.prevent="createFacility" class="space-y-4 mb-8">
                    <div>
                        ...
                        <input v-model="addForm.name" type="text"
                            class="border w-full py-2 px-2 rounded-md mt-1 border-stone-300 hover:border-cyan-600"
                    ...
                    <div class="w-full  flex justify-center items-center flex-col space-y-4 pt-3">
                        ...
                        <div>
                            <button type="submit" class="bg-cyan-600 rounded-lg 
                        px-4 py-2 text-white hover:bg-cyan-700 transition">เพิ่มประเภทที่พัก</button>
                        </div>
                    </div>
                </form>
                ...
                <div v-if="isEditing" ref="editFormSection"
                    class="mt-8 border-t max-w-2xl mx-auto rounded-lg border px-4 py-5 mb-8 shadow-lg bg-cyan-50">
                    <h2 class="text-xl font-semibold mb-2 text-cyan-700 text-center">แก้ไขประเภททที่พัก</h2>
                    <form @submit.prevent="updateFacility" class="space-y-4">
                        <div>
                            <label class="font-semibold text-sm">ประเภทที่พัก</label>
                        ...
                        <div class="pt-10 space-x-2 flex justify-center">
                            <button type="submit" class="bg-cyan-400 rounded-lg 
                        px-4 py-2  hover:bg-cyan-500 transition">อัปเดตข้อมูล</button>
...
const createFacility = async () => {
    try {
        ...
        await axios.post('http://localhost:9999/SleepGun/typeHotel/create', fd)
...
const updateFacility = async () => {
    try {
        ...
        await axios.put(`http://localhost:9999/SleepGun/typeHotel/update/${selectedId.value}`, fd)
...
const deleteFacility = async (id) => {
    ...
    try {
        await axios.delete(`http://localhost:9999/SleepGun/typeHotel/delete/${id}`)
        fetch()
    } catch (err) {
        console.error(err)
    }
